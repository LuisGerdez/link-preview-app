...
export default function TwitterPreviewCard({ meta }: { meta: MetaTags }) {
  if (meta.twitter_card === 'summary_large_image' && meta.og_image) {
    return (
  ...
  return (
    <div className="flex items-stretch min-w-md max-w-md border border-[rgb(229,231,235)] bg-white rounded-xl overflow-hidden shadow-sm" style={{ fontFamily: 'Segoe UI, Arial, sans-serif' }}>
      ...
      <div className="flex-1 min-w-0 px-4 py-3 hover:bg-[oklch(96.7%_0.003_264.542)]">
        ...
        <div className="font-semibold text-[16px] text-[rgb(17,24,39)] leading-tight truncate">{meta.og_title || meta.html_title || "(No title)"}</div>
        <div className="text-[15px] text-[rgb(55,65,81)] leading-snug truncate">{meta.og_description || meta.meta_description || "(No description)"}</div>
      </div>
    </div>
  );
}